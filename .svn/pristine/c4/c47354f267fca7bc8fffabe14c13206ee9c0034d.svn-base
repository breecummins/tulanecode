...
class Stokeslet2DReg2():
	...
	def makeMatrix2DWithIntegrals(self,obspts,nodes):
		if nodes.shape[1] != 2:
			raise ValueError('Requires two dimensional inputs.')
	...
	def makeNodeCoeffs(self,nodes):
		...
		difnod = nodes[:-1] - nodes[1:]
		a2, b2, c2 = self.makeNChelper(difnod[:-1])
		a2p1, b2p1, c2p1 = self.makeNChelper(difnod[1:])
...
def exactSoln(obspts,rad=0.25,mu=1.0,BC=nm.array([1,0])):
...
if __name__ == '__main__':
	...
	#now make two line segment example
	
	#single blob for table, single far and near points
	mu = 1.0
	rad = 0.25
	farp = 0.5*nm.array([nm.cos(0.7),nm.sin(0.7)])
	nearp = 0.26*nm.array([nm.cos(0.7),nm.sin(0.7)])
	...
	# 			origu = nm.append(origu,origvel[0::2])	
	# 			origv = nm.append(origv,origvel[1::2])	
	# 	fname = 'StokesCylTest_exactintegralsANDorig_patches_singleblobfortable_N'+ (4-len(str(N)))*'0' + str(N) + '.mat'                	
	# 	mf.write(fname,{'u':u,'v':v,'origu':origu,'origv':origv,'exactu':exactu,'exactv':exactv,'obspts':obspts,'origbps':origblobparams,'eintbps':eintblobparams})
	...
	# 	nodes = discretizeCircle(N,rad)
	# 	BCs = nm.asarray(N*[1,0])
	# 	origblobparams = nm.linspace(2*nm.pi/(250*N),2*nm.pi/(N*4),40)
	# 	eintblobparams = nm.linspace((2*nm.pi/(125*N))**2,(2*nm.pi/(N*2))**2,40)
	# 	u = nm.empty((0,))
	# 	v = nm.empty((0,))
	# 	origu = nm.empty((0,))
	# 	origv = nm.empty((0,))
	...
	# 			origu = nm.append(origu,origvel[0::2])	
	# 			origv = nm.append(origv,origvel[1::2])	
	# 	fname = 'StokesCylTest_exactintegralsANDorig_patches_N'+ (4-len(str(N)))*'0' + str(N) + '.mat'                	
	# 	mf.write(fname,{'u':u,'v':v,'origu':origu,'origv':origv,'exactu':exactu,'exactv':exactv,'obspts':obspts,'origbps':origblobparams,'eintbps':eintblobparams})
	# 
