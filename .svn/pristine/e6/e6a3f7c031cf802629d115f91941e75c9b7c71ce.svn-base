...
C
     ...
     .        SC, V1(3), V2(3), V3(3), VLAT, VLON, VNRM
      REAL    AREAS
C
      PARAMETER (NMAX=1000000, NTMX=2*NMAX, NT6=6*NMAX,
     .           LWK=2*NMAX, NCOL=NMAX, NROW=16)
C
...
C
      ...
      OPEN (LOUT,FILE='results',STATUS= 'UNKNOWN')
      OPEN (LPLT,FILE='deltri.eps',STATUS= 'UNKNOWN')
      OPEN (LPLV,FILE='voronoi.eps',STATUS= 'UNKNOWN')
C
...
C *** Read a data set:
      ...
      PRINT *,"# of nodes = ",N
      DO NODE = 1,N
         READ(16,*) K,X(NODE),Y(NODE),Z(NODE)
      ENDDO
      CLOSE(16)
C
...
C
  ...
  320 FORMAT (//5X,'Output from BNODES:  ',I8,
     .        ' boundary nodes,  ',I8,
     .        ' arcs,  ',I8,' triangles.'//)
C
...
C
  ...
  330   FORMAT (5X,'Subroutine DELARC not tested:'/
     .          5X,'Nodes ',I8,' and ',I8,' do not form a ',
...
C
  ...
  335   FORMAT (/5X,'A Voronoi diagram plot file was ',
     .              'successfully created.'/)
      ...
      N0 = 1
      WRITE (LOUT,340) N0
  340 FORMAT (17X,'Voronoi region for node ',I8//
     .        5X,'Triangle',6X,'XXXXXX',6X,'YYYYYY',
     .        6X,'ZZZZZZ',5X,'Circumradius'/)
C
...
C
    ...
    7 LP = LPTR(LP)
        ...
        NV = NV + 1
        IWK(NV) = KT
C        CALL SCOORD (XC(KT),YC(KT),ZC(KT), VLAT,VLON,VNRM)
C        VLAT = VLAT/SC
C        VLON = VLON/SC
        WRITE (LOUT,345) KT,XC(KT),YC(KT),ZC(KT),RC(KT)
C        WRITE (LOUT,345) KT, VLAT, VLON, RC(KT)
        IF (LP .NE. LPL) GO TO 7
  345 FORMAT (9X,I8,1X,F11.6,2X,F11.6,2X,F11.6,2X,F11.6)
C
...
C#500 FORMAT (//5X,'*** Input data set invalid ***'/)
  ...
  550 FORMAT (//5X,'*** Error in CRLIST:  IER = ',I1,
     .        ' ***'/)
  560 FORMAT (//5X,'*** Error in INSIDE:  node ',I8,' is ',
